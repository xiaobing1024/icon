...
@section('content')
    <div class="page">
        <div class="weui-flex fixhead" style="align-items:baseline">
            <button class="weui-btn weui-btn_mini weui-btn_primary" style='margin-left:15px;color:#fff;box-shadow: 0 1.5px 4px rgba(0, 0, 0, 0.24), 0 1.5px 6px rgba(0, 0, 0, 0.12);' v-on:click="typeChange">切换</button>
            <span style='color:#fff;line-height:29px;' v-cloak>@{{ t }}</span>
            <button id="copy" class="weui-btn weui-btn_mini weui-btn_warn" v-bind:class="cancopy" style='margin-right:15px;color:#fff;box-shadow: 0 1.5px 4px rgba(0, 0, 0, 0.24), 0 1.5px 6px rgba(0, 0, 0, 0.12);' v-bind:data-clipboard-text="copyText">复制</button>
        </div>

        <div class="page__bd page__bd_spacing">
            <div class="pick_ball" v-for="(i, index) in red_line">
                <template v-for="(j, z) in line_count">
                    <div v-if="(index * 7 + z) < reds.length" class="item" v-bind:class="reds[index * 7 + z].checked ? 'is_checked' : ''">
                        <label class="checkbox"><input type="checkbox" hidden="false" v-bind:value="reds[index * 7 + z].name" v-model="reds_p" @change="redChange(index * 7 + z)"/>@{{ reds[index * 7 + z].name }}</label>
                    </div>
            ...
            <div class="pick_ball" v-for="(i, index) in blue_line">
                <template v-for="(j, z) in line_count">
                    <div v-if="(index * 7 + z) < blues.length" class="item item_blue" v-bind:class="blues[index * 7 + z].checked ? 'is_checked_blue' : ''">
                        <label class="checkbox"><input type="checkbox" hidden="false" v-bind:value="blues[index * 7 + z].name" v-model="blues_p" @change="blueChange(index * 7 + z)"/>@{{ blues[index * 7 + z].name }}</label>
                    </div>
        ...
        <div class="weui-footer weui-footer_fixed-bottom">
            ...
            <button id="copy" class="weui-btn weui-btn_mini weui-btn_default" style="margin-right:15px;box-shadow: 0 1.5px 4px rgba(0, 0, 0, 0.24), 0 1.5px 6px rgba(0, 0, 0, 0.12);">清空</button>
        </div>
    </div>
@endsection
